/*
  # Initial Database Schema for EPKalk

  1. New Tables
    - `organizations` - Company/organization information
    - `users` - User profiles linked to auth.users
    - `customers` - Customer information per organization
    - `projects` - Projects belonging to customers
    - `calculators` - Calculation sheets for projects
    - `user_settings` - User-specific settings and preferences

  2. Security
    - Enable RLS on all tables
    - Add policies for organization-based access control
    - Users can only access data from their organization
    - Admins and managers have additional permissions

  3. Features
    - Custom enum types for user roles and project status
    - Automatic timestamp updates
    - JSON storage for flexible data (entries, summary, settings)
