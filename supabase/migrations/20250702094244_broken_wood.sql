/*
  ...
  This migration fixes the user table policies to avoid recursion issues
  and ensures proper access control for different user roles.
  
  1. Safely drops existing policies and functions with dependencies
  2. Creates helper functions to avoid recursion in RLS policies
  3. Recreates all necessary policies for users and organization_invitations tables
*/
...
-- Recreate organization_invitations policies that depend on the functions
-- (These were dropped when we used CASCADE above)
